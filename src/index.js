...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/health") {
      const result = await health(url, env);
      return json(result);
    }
...
async function handleUpdate(
  ...
  if (cmd === "/start") {
    ...
    await send(
      ...
      "اخبار تازه از منابع مختلف " +
      "دریافت، ترجمه و منتشر می‌شوند."
    );
...
async function tgApi(
  ...
  try {
    ...
    throw new Error(
      `Telegram HTTP ${r.status}: ${raw.slice(0, 300)}`
    );
  }
    ...
    const r =
      await fetch(
        feed.url,
        {
          headers: {
            "User-Agent":
              "Mozilla/5.0 (compatible; Nabz/1.0)",
            "Accept":
